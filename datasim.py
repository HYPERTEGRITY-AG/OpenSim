...
def do_delete(session, lock, args, max_id_length):
    ...
    for i in range(args.delete[0], args.delete[1] + 1):
        ...
        if args.protocol == helper.PROTOCOL_NGSI:
            ...
            thing_id, ms = sensor_things.get_thing_id(session, host, thing_name, args.x_api_key)
            if thing_id == sensor_things.INVALID_ID:
                not_deleted += 1
                resp = requests.Response()
                resp.status_code = 404
            else:
                resp = sensor_things.delete_thing(session, host, thing_id, args.x_api_key)
        ...
        with lock:
            if resp is None:
                ...
                error_as_string = str(resp.status_code) + " " + " ".join(resp.text.split())[0:120]
            ...
            if args.verbose:
                if connection_error:
                    print('%s%s  ??? %s' % (delimiter, helper.create_id(i, args.prefix, args.postfix, max_id_length),
                                            error_as_string), end="")
                else:
                    print('%s%s  %3i %s' % (delimiter, helper.create_id(i, args.prefix, args.postfix, max_id_length),
                                            resp.status_code,
                                            " ".join(resp.text.split())[0:120]), end="")
                delimiter = "\n"
...
def do_send(mqtt_client, session, lock, args, offset, max_id_length):
    ...
    if args.unlimited:
        args.num_messages = 1000000000  # there is no infinite mode :-) - but don't send more than one billion messages!
    ...
    for i in range(args.num_messages):
        ...
        if args.protocol == helper.PROTOCOL_NGSI:
            ...
            if args.insert_always:
                ...
                resp, payload = ngsi.do_patch(session,
                                              host,
                                              first_id,
                                              headers,
                                              args)
                ...
                if args.verbose:
                    if resp is None:
                        print('%s  ???  ---- Connection Error!' %
                              helper.create_id(first_id, args.prefix, args.postfix, max_id_length))
                    else:
                        message = " ".join(resp.text.split())[0:120]
                        # There is this funny thing that Orion sometimes tells us about 400 ParseError
                        if resp.status_code == 400:
                            message += ("\nPayload was:\n" + payload)
                        ...
                        print('%s  %3i  %4i %s' % (helper.create_id(first_id, args.prefix, args.postfix, max_id_length),
                                                   resp.status_code,
                                                   ms,
                                                   message))
        else:
            ...
            if args.datastream_id is not None:
                ...
                if thing_id != sensor_things.INVALID_ID:
                    ...
                    with lock:  # since we might be running in more than one thread, use lock!
                        ...
                        if thing_id == sensor_things.INVALID_ID:
                            thing_name = helper.create_id(first_id, args.prefix, args.postfix, 0)
                            ...
                            if resp is None:
                                ...
                                ms = int(resp.elapsed.total_seconds() * 1000)
                                thing_id, resp = sensor_things.create_thing(session,
                                                                            host,
                                                                            thing_name,
                                                                            args.indent,
                                                                            args.x_api_key)
            ...
            if okay and thing_id != sensor_things.INVALID_ID and thing_id != sensor_things.ERROR:
                if args.numbers is not None:
                    for number in args.numbers:
                        attribute_args = number[0].split(',')

                        # shortcut: if at this point data_stream_id != INVALID_ID, then we only have ONE
                        #           attribute at all! So, no need to find out data_stream_id...create
                        #           an Observation immediately instead
                        if args.datastream_id is None and data_stream_id == sensor_things.INVALID_ID:
                            with lock:  # since we might be running in more than one thread, use lock!
                                ...
                                if data_stream_id == sensor_things.ERROR:
                                    ...
                                    data_stream_id, ms2 = sensor_things.create_data_stream(session,
                                                                                           host,
                                                                                           thing_id,
                                                                                           data_stream_name,
                                                                                           args.indent,
                        ...
                        resp = sensor_things.create_observation(mqtt_client,
                                                                session,
                                                                host,
...
def wait_for_delete_thread(args):
    ...
    while not ready:
        ...
        if overall_messages > 0 and not args.verbose:
            if args.unlimited:
                print("\rMessages sent: %i with %i not found and %i other error(s)      " % (
                      overall_messages,
                      not_deleted,
                      errors),
                      end="",
                ...
                print("\rMessages sent: %i (%i%%) with %i not found and %i other error(s)      " % (
                      overall_messages,
                      (overall_messages / (args.delete[1] - args.delete[0] + 1)) * 100,
                      not_deleted,
                      errors),
                      end="",
                      flush=True)
...
def create_and_start_delete_thread(session, lock, args, max_id_length):
    ...
    delete_thread = Thread(target=do_delete, args=(session,
                                                   lock,
                                                   args,
                                                   max_id_length))
...
def create_send_threads(args, mqtt_client, session, lock, max_id_length):
    ...
    for i in range(args.num_threads):
        t = Thread(target=do_send, args=(mqtt_client,
                                         session,
                                         lock,
                                         args,
                                         offset,
...
def wait_for_send_threads(limit_time, verbose, msg_num, unlimited):
    ...
    while not ready:
        ...
        if not verbose:
            output.print_messages_send(overall_messages, errors, overall_time, temp_ms, msg_num, unlimited)
...
def handle_send(args, session, lock, msg_num, max_id_length):
    ...
    mqtt_client = sensor_things.init_mqtt(helper.create_host_url(args.server)) \
    ...
    if args.verbose:
        output.print_messages_send(overall_messages, errors, overall_time, ms, msg_num, args.unlimited)
        print("")
