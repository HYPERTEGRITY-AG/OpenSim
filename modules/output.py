...
def print_version():
    print("opensim.py, Copyright (c) 2021 Will Freitag, Version %s" % helper.VERSION, flush=True)
...
def print_id_used(args, msg_num):
    if args.static_id:
        print("ID \"%s\" will be used for all messages." %
              helper.create_id(args.first_id, args.prefix, args.postfix, 0),
              flush=True)
    else:
        if msg_num == 1:
            print("The used ID will be \"%s\"." %
                  helper.create_id(args.first_id, args.prefix, args.postfix, 0),
                  flush=True)
        elif args.unlimited:
            print("The used ID will be starting at \"%s\" and increase continuously." %
                  helper.create_id(args.first_id, args.prefix, args.postfix, 0),
                  flush=True)
        else:
            print("The used ID will be from \"%s\" to \"%s\"." %
                  (helper.create_id(args.first_id, args.prefix, args.postfix, 0),
                   helper.create_id(args.first_id + msg_num - 1, args.prefix, args.postfix, 0)),
                  flush=True)
    ...
    if args.protocol == helper.PROTOCOL_NGSI:
        ...
        print("The payload will look like:\nThing:\n%s\n\nDatastream: \n%s\n\nObservation: \n%s" %
              (helper.create_thing_payload(thing_name, args.indent),
               ...
               helper.create_observation_payload(1.5, args.indent)), flush=True)


def print_will_delete(args):
    print("Will delete %i contexts from \"%s\" to \"%s\"." %
