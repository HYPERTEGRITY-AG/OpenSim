...
def do_delete(session, lock, args, max_id_length):
    ...
    for i in range(args.delete[0], args.delete[1] + 1):
        ...
        with lock:
            ...
            if args.verbose:
                if connection_error:
                    print('%s%s  ??? %s' % (delimiter, helper.create_id(i,
                                                                        args.prefix,
                                                                        args.postfix,
                                                                        max_id_length),
                                            error_as_string), end="")
                else:
                    print('%s%s  %3i %s' % (delimiter, helper.create_id(i,
                                                                        args.prefix,
                                                                        args.postfix,
                                                                        max_id_length),
                                            resp.status_code,
            ...
            with lock:
                if not okay:
                    ...
                    if resp is None:
                        ...
                        error_as_string = str(resp.status_code) + \
                                          " " + \
                ...
                if args.verbose:
                    if resp is None:
                        ...
                        print('%s  %3i  %4i %s' % (helper.create_id(first_id,
                                                                    args.prefix,
                                                                    args.postfix,
                                                                    max_id_length),
                                                   resp.status_code,
            ...
            if okay and thing_id != sensor_things.INVALID_ID and thing_id != sensor_things.ERROR:
                if args.numbers is not None:
                    for number in args.numbers:
                        ...
                        if args.datastream_id is None and \
                                data_stream_id == sensor_things.INVALID_ID:
...
def wait_for_send_threads(limit_time, verbose, msg_num, unlimited):
    ...
    while not ready:
        ...
        if not verbose:
            output.print_messages_send(overall_messages,
                                       errors, overall_time, temp_ms, msg_num, unlimited)
